# Edge-cloud v2 machine contract
...
- Header schemas model application operation headers, not transport-managed headers such as Host or User-Agent. Names are case-insensitive. Wire numeric IDs/revisions are decimal strings. No Authorization/JWT header belongs in a machine request.
- `null` in an operation's request/response schema entry means **no HTTP body**, not a serialized JSON null. This applies to GET requests, heartbeat, 204 and 304.
- Every operation also uses the common error envelope for 429 and server failures (all 5xx). `operations.json` enumerates common 500/502/503/504 examples; the retry rule covers other 5xx as well. Timeout/network failure preserves pending data and uses backoff with jitter, subject to activation's non-repeatable rule.

### Concrete response representation

The immutable configuration contains `gateway_id`, `property_id` and `slots`; each slot has `slot_id`, `logical_node_id`, `irrigation_area_id`, and nullable `hardware_profile_code`. The live overlay has those three slot identity fields, `binding_status`, and nullable `current_binding` / `pending_binding`. A visible binding carries `candidate_id`, `uid`, `serial`, `submitted_at`, and nullable `confirmed_at`.

`unbound` has neither binding; `pending_initial` has only pending; `confirmed` has only current; `pending_reassignment` has both, with current remaining effective until confirmation. Snapshot and overlay slot identities match. Physical identifiers occur only in binding operations/overlay, never telemetry.

Telemetry success metadata is `{id, node_id, timestamp, created_at}`. NDVI success returns the separate NDVI event shape. Update confirmation success returns `{id, authorization_id, image_version, image_digest, technician_confirmed_at, result}`. Exact replays preserve the original result. Image digest is an opaque nonempty immutable identity agreed by both sides; the fixture uses a synthetic SHA-256 identity, not an image or an update instruction.
...
python scripts/integration/validate_v2_contract.py
python -m unittest discover -s scripts/integration -p 'test_v2_contract.py'
```
